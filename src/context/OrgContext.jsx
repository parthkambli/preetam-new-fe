...
export function OrgProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token && storedOrgs) {
      ...
      setAvailableOrgs(parsedOrgs);
      
      if (currentOrgId) {
        const org = parsedOrgs.find(o => o.id === currentOrgId);
        setCurrentOrg(org || parsedOrgs[0]);
      } else {
        setCurrentOrg(parsedOrgs[0]);
      }
  ...
  const login = (organizations, defaultOrg, userData) => {
    setAvailableOrgs(organizations);
    setCurrentOrg(defaultOrg);
    setIsAuthenticated(true);
    
    // IMPORTANT: Save user data (contains role: 'SchoolStaff' or 'FitnessStaff')
    if (userData) {
      setUser(userData);
      localStorage.setItem('user', JSON.stringify(userData));
    }
