...
const generatePassword = (fullName = "", mobileNumber = "") => {
  const cleanName = fullName.trim().replace(/\s+/g, "");
  const namePart = cleanName.substring(0, 3) || "Emp";
  const mobilePart = mobileNumber.slice(-4) || "1234";
...
export default function AddStaff() {
  ...
  // Fetch roles and employment types
  useEffect(() => {
    api.fitnessStaff.getRoles()
      .then((res) => setRoles(res.data.map((r) => ({ value: r.name, label: r.name }))))
      .catch(() => {});

    api.fitnessStaff.getEmploymentTypes()
      .then((res) => setEmpTypes(res.data.map((t) => ({ value: t.name, label: t.name }))))
      .catch(() => {});
  }, []);


  useEffect(() => {
  if (form.fullName?.trim() && form.mobileNumber?.length >= 4) {
  ...
  const validate = () => {
    ...
    else if (form.mobileNumber.length !== 10) e.mobileNumber = 'Mobile number must be exactly 10 digits';

    if (!form.joiningDate) e.joiningDate = 'Joining date is required';
    // Removed future date restriction for joiningDate
    ...
    // Updated validation for emergency contact mobile
    if (form.emergencyContactMobile && form.emergencyContactMobile.length > 0) {
      if (form.emergencyContactMobile.length !== 10) {
        e.emergencyContactMobile = 'Emergency contact number must be exactly 10 digits';
      }
    }
  ...
  //   // Auto generate password if user left it empty
  //   const isPasswordAutoGenerated = !finalPassword?.trim();
  //   if (isPasswordAutoGenerated) {
  //     finalPassword = generatePassword();
  //     setGeneratedPasswordInfo(finalPassword);
  //   }
  ...
  //     if (key === 'password') {
  //       formData.append(key, finalPassword);
  ...
  //   try {
  //     await api.fitnessStaff.create(formData);
...
//   // Send all form fields normally (including 'role' which goes to FitnessStaff.role)
//   Object.entries(form).forEach(([key, value]) => {
//     if (key === 'password') {
//       formData.append(key, finalPassword);
...
//   // Keep 'profilePhoto' since you confirmed upload is working with it
//   if (photoFile) {
//     formData.append('profilePhoto', photoFile);
//   }
...
let finalPassword = form.password;
  ...
  if (Object.keys(validationErrors).length > 0) {
    ...
    // Show toast for the first error (cleanest approach)
...
///////
        ...
        <div className="mb-4 px-4 py-2.5 bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg flex items-center gap-2">
          ...
          {generatedPasswordInfo && (
            <div className="mt-1 text-xs">
              Auto-generated Password: <span className="font-mono font-medium bg-green-100 px-1.5 py-0.5 rounded">{generatedPasswordInfo}</span>
            </div>
          )}
        </div>
      )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="e.g. Suresh Patil"
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.fullName ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                value={form.role}
                onChange={handleChange}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 appearance-none focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.role ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              >
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                value={form.gender}
                onChange={handleChange}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 appearance-none focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.gender ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              >
                <option value="">Select Gender</option>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                max={today}
                min={minDOB}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.dateOfBirth ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                onChange={handleChange}
                // Removed max={today} so future dates are allowed
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.joiningDate ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
              {errors.joiningDate && <p className="text-xs text-red-500 mt-0.5">{errors.joiningDate}</p>}
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={handleChange}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 appearance-none focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.employmentType ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              >
              ...
              {errors.employmentType && <p className="text-xs text-red-500 mt-0.5">{errors.employmentType}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                placeholder="Used as Login ID"
                maxLength={10}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.mobileNumber ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
              {errors.mobileNumber && <p className="text-xs text-red-500 mt-0.5">{errors.mobileNumber}</p>}
            </div>
  ...
  placeholder="Auto-generated from name + mobile"
          ...
          {/* Emergency Contact - Fixed to 10 digits */}
          <SectionDivider title="Emergency Contact" />
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <InputField
              label="Contact Name"
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                maxLength={10}
                className={`w-full border rounded-lg px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all ${errors.emergencyContactMobile ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              />
              {errors.emergencyContactMobile && (
                <p className="text-xs text-red-500 mt-0.5">{errors.emergencyContactMobile}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto px-6 py-2.5 text-sm bg-[#000359] hover:bg-[#000280] active:bg-[#00023d] text-white rounded-lg transition-colors font-semibold shadow-sm disabled:opacity-60"
            >
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        required={required}
        placeholder={placeholder}
        className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-gray-800 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#000359]/30 focus:border-[#000359] bg-white transition-all"
      />
    </div>
  );
}
