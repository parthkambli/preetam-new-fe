...
//     setLoading(true);
//     try {
//       const res = await api.schoolStaffPanel.getAllotments();
...
export default function AllotFees({ onSuccess }) {
    ...
    const loadData = async () => {
      ...
      try {
        const [studentsRes, feeTypesRes, allotmentsRes] = await Promise.all([
          api.schoolStaffPanel.getStudents(),           // ← Best API
          api.schoolStaffPanel.getFeeTypes(),
          api.schoolStaffPanel.getAllotments(),
        ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.schoolStaffPanel.allotFee(payload);
      toast.success('Fee allotted successfully!');
