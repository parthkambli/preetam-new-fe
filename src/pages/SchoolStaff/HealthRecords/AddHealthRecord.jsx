...
export default function AddUpdateHealthRecord() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await api.schoolStaffPanel.createHealthRecord(formData);
        toast.success('Health record saved successfully');
      }
  ...
  const handleCancel = () => navigate('/school-staff/health-records');

  const loadStudentOptions = async (inputValue) => {
    try {
      const params = inputValue ? { searchName: inputValue } : { limit: 10 };
      const res = await api.schoolStaffPanel.getStudents(params);
