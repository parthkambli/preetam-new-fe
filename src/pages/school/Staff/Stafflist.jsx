...
export default function StaffList() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      if (debouncedFilters.search?.trim()) params.name = debouncedFilters.search.trim();
      if (debouncedFilters.mobile?.trim()) params.mobile = debouncedFilters.mobile.trim();
      if (debouncedFilters.role) params.role = debouncedFilters.role;        // ← This is the key fix
      if (debouncedFilters.status) params.status = debouncedFilters.status;
  ...
  // Fetch staff when debounced filters change
  useEffect(() => {
    fetchStaff();
  }, [debouncedFilters]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3">
        ...
        >
          <option value="">All Roles</option>
          {roles.map((role) => (
            <option key={role._id} value={role.name}>   {/* Changed to role.name */}
              {role.name}
            </option>
          ))}
        </select>
