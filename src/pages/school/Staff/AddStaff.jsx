...
function InputField({ label, name, value, onChange, type = 'text', required, placeholder, error }) {
  return (
...
export default function AddStaff() {
  ...
  const handlePhotoChange = (e) => {
    ...
    if (photoError) {
      ...
      e.target.value = ''; // IMPORTANT: reset input so user can try again
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      // Scroll to top of form so user sees the red fields
    ...
    try {
      ...
      formData.append('loginId', form.loginId.trim() || form.mobile.trim());
  ...
  const handleApiError = (err) => {
    // Case 1: No response at all — user is offline or server is completely down
    if (!err.response) {
    ...
    switch (status) {
      ...
      case 413:
        // File too large (server-side limit, e.g. multer or nginx)
  ...
  // Count errors to show a summary banner
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {/* Error summary banner — shows when form has been submitted with errors */}
      {errorCount > 0 && (
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-7 space-y-6">
          ...
          <div className="flex items-center gap-5">
            ...
            {/* IMPORTANT: accept only images, and note the size limit to the user */}
            <input
              ref={fileRef}
              type="file"
