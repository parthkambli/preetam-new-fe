...
export default function AddPayments({ onSuccess }) {
  const [payments, setPayments] = useState([]);
  const [students, setStudents] = useState([]);
  const [allotmentsForStudent, setAllotmentsForStudent] = useState([]);
  ...
  // ── Load Allotments when Student is selected ─────────────────
  useEffect(() => {
    if (!form.studentId) {
      setAllotmentsForStudent([]);
      return;
    }

    const loadAllotments = async () => {
      try {
        const res = await api.fees.getAllotments({ studentId: form.studentId });
        const data = res.data || [];

        // Ensure we only show allotments for the selected student
        const filtered = data.filter(allotment => 
          allotment.studentId?._id === form.studentId || 
          allotment.studentId === form.studentId
        );

        setAllotmentsForStudent(filtered);
      } catch (err) {
        console.error('Failed to load allotments:', err);
        toast.error('Failed to load fee allotments for this participant');
        setAllotmentsForStudent([]);
      }
    };
  ...
  const handleAllotmentSelect = (option) => {
    ...
    const selectedAllotment = allotmentsForStudent.find(a => a._id === option.value);

    setForm(prev => ({
      ...prev,
      allotmentId: option.value,
      // Pre-fill with remaining amount if possible
      amount: selectedAllotment?.remainingAmount || selectedAllotment?.amount || '',
    }));
  };

  const handleSave = async () => {
    if (!form.studentId) return toast.error('Please select a participant');
    if (!form.allotmentId) return toast.error('Please select a fee allotment');
    ...
    try {
      ...
      await api.fees.addPayment(payload);
  ...
  const studentOptions = students.map((student) => ({
    ...
    label: `${student.fullName || student.name} ${
      student.studentId || student.rollNo ? `(${student.studentId || student.rollNo})` : ''
    }`,
  }));

  const allotmentOptions = allotmentsForStudent.map((allotment) => ({
    value: allotment._id,
    label: `${allotment.feeTypeId?.description || allotment.description || 'Fee'} - ₹${allotment.amount} (${allotment.feePlan || 'Annual'})`,
  }));
    ...
    <div className="space-y-5">
      ...
      <div className="border border-gray-200 rounded-lg p-5 bg-white space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
          ...
          {/* Fee Allotment */}
          <div>
            <label className="block text-xs font-semibold text-[#1e3a8a] mb-1">Fee Allotment</label>
            <Select
              options={allotmentOptions}
              onChange={handleAllotmentSelect}
              value={allotmentOptions.find((opt) => opt.value === form.allotmentId) || null}
              placeholder={form.studentId ? "Select allotted fee..." : "Select participant first"}
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full min-w-[900px] border-collapse">
          <thead>
            <tr className="bg-[#1e3a8a]">
              {['Participant', 'Fee Description', 'Type', 'Amount', 'Status', 'Payment Date', 'Payment Mode'].map((h) => (
                <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-white whitespace-nowrap">
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="py-10 text-center text-gray-500">Loading payments...</td>
              </tr>
            ) : filteredPayments.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-10 text-center text-gray-400">No payment records found</td>
              </tr>
            ) : (
